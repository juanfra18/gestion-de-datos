...
SELECT  ti.TIEMPO_ANIO, 
		...
		CAST(SUM(ven.VENTA_CANT_VENTAS_AGRUPADAS) * 100 / (SELECT SUM(an.ANUNCIO_CANT_ANUNCIOS_AGRUPADOS)
														FROM GUISO_DE_LENTEJAS.BI_Hecho_Anuncio an
														JOIN GUISO_DE_LENTEJAS.BI_Tiempo ti2 ON (ti2.TIEMPO_CODIGO = an.ANUNCIO_TIEMPO_INICIO)
														WHERE ti2.TIEMPO_ANIO = ti.TIEMPO_ANIO
														GROUP BY ti2.TIEMPO_ANIO) AS numeric(6,2)) AS 'Porcentaje del Total de Anuncios'
		...
		GROUP BY ti.TIEMPO_ANIO, su.SUCURSAL_NOMBRE, ret.RANGO_ETARIO_AGENTE_DESCRIPCION
UNION
SELECT  ti.TIEMPO_ANIO, 
		su.SUCURSAL_NOMBRE, 
		ret.RANGO_ETARIO_AGENTE_DESCRIPCION, 
		CAST(SUM(al.ALQUILER_CANT_ALQUILERES_AGRUPADOS) / (SELECT SUM(an.ANUNCIO_CANT_ANUNCIOS_AGRUPADOS)
															FROM GUISO_DE_LENTEJAS.BI_Hecho_Anuncio an
															JOIN GUISO_DE_LENTEJAS.BI_Tiempo ti2 ON (ti2.TIEMPO_CODIGO = an.ANUNCIO_TIEMPO_INICIO)
															WHERE ti2.TIEMPO_ANIO = ti.TIEMPO_ANIO
															GROUP BY ti2.TIEMPO_ANIO) AS numeric(6,2)) AS 'Porcentaje del Total de Anuncios'
